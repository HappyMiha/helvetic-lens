...
import { hazardEmailCopy } from "@/lib/hazard-email-copy";
...
function EmailForm({
  ...
  const path = `/hazard-watch/monitors/${monitorId}`;
            ...
            <section aria-live="polite">
              ...
              <ul>
                {preview.items.map((item) => (
                  <li key={`${item.event_id}:${item.revision}`}>
                    <a href={item.href}>
                      {c.version} · {item.detected_at} · {item.revision}
                    </a>
                  </li>
                ))}
              </ul>
