...
export function InboxPageNavigation({ page, newestHref, nextHref, busy = false, position = "top" }: {
  ...
  const { t } = useI18n();
  return (
    <nav className="rounded-lg border border-border bg-background p-4 my-4 text-sm break-words" aria-label={t(position === "top" ? "inboxPaging.navigationTop" : "inboxPaging.navigationBottom")} data-inbox-navigation>
      {page && <p className="m-0" role={position === "top" ? "status" : undefined}>{t("inboxPaging.counts", { shown: page.total_events, scanned: page.scanned_event_count })}</p>}
