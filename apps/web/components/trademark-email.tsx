...
import { trademarkEmailCopy } from "@/lib/trademark-email-copy";
...
export function TrademarkEmail({
  monitorId,
  canManage,
  archived,
  changed,
  ...
  archived: boolean;
  changed: () => void;
  onAccessFailure: (problem: unknown) => void;
}) {
  const { locale } = useI18n();
  const [open, setOpen] = useState(false);
  return (
    <details
      data-trademark-email
...
function EmailForm({
  ...
  const path = `/trademark-watch/monitors/${monitorId}`;
  ...
  }, [path, revision, c.failed, onAccessFailure]);
            ...
            <section aria-live="polite">
              ...
              <ul>
                {preview.items.map((item) => (
                  <li key={item.href}>
                    <a href={item.href}>{item.detected_at}</a>
                  </li>
                ))}
              </ul>
