...
export function MaterialChanges({clusters, changes, state, onStateChange, onEvidence}: {
  clusters: Cluster[]; changes: Change[]; state: Position;
  ...
  const rows = useMemo(() => {
    ...
    return clusters.map((cluster, index) => {
      ...
      return {cluster, index, items, search: items.flatMap(item => [item.old?.text || "", item.new?.text || ""]).join(" ").normalize("NFKC").toLocaleLowerCase()};
    });
  }, [clusters, changes]);
  const query = state.query.trim().normalize("NFKC").toLocaleLowerCase();
  const matching = query ? rows.filter(row => row.search.includes(query)) : rows;
  ...
  return <section className="material-reader" data-material-reader aria-label={t("materialPage.title")}>
    ...
    <div className="semantic-clusters">
      {shown.map(({cluster,index,items}) => {
        const first = items[0];
        return <article key={cluster.id} data-material-group={cluster.id}>
          <div className="semantic-cluster-heading"><h4>{t("compare.changeGroup", {number:number(index + 1)})}</h4><span>{t("compare.exactChangeCount", {count:number(cluster.change_ids.length)})}</span></div>
