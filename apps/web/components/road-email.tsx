...
import { roadEmailCopy } from "@/lib/road-email-copy";
...
function EmailForm({
  ...
  const path = `/road-watch/monitors/${monitorId}`;
              ...
              <ul>
                {preview.items.map((item) => (
                  <li key={`${item.event_id}:${item.sequence}`}>
                    <a href={item.href}>
                      {c.version} · {item.detected_at} · {item.sequence}
                    </a>
                  </li>
                ))}
              </ul>
