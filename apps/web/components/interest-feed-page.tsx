...
export function InterestFeedPage() {
  ...
  async function setReadingState(item: FeedEvent, state: string) {
    ...
    setNotice("");
    try {
      await api(`/interest-feed/events/${item.event_id}/state`, {
        method: "PATCH",
  ...
  return (
    <Shell section={t("nav.today")}>
      <header className="page-heading">
        <div>
          <h1>{t("feed.title")}</h1>
          <p className="muted max-w-3xl">{t("feed.body")}</p>
        </div>
        <Button
          variant="outline"
          onClick={() => {
            router.replace(href({ cursor: "" }));
            setPollenRefresh((value) => value + 1);
            void invalidateResources(resourceTag("impact-inbox"));
          }}
        >
          <RefreshCw size={16} />
      ...
      <TenderToday key={`tender-${pollenRefresh}`} />
      ...
      <details className="rounded-lg border px-4 text-sm">
      ...
      <div className="grid grid-cols-2 gap-4 my-5">
        <label className="text-sm">
          {t("feed.detected")}
          <select
            className="interest-feed-control w-full mt-2 min-h-[44px]"
            value={params.get("period") || "all"}
            onChange={(event) =>
              router.push(href({ period: event.target.value, cursor: "" }))
            }
          >
        ...
        <label className="text-sm">
          ...
          <select
            ...
            onChange={(event) =>
              router.push(href({ state: event.target.value, cursor: "" }))
            }
          >
      ...
      {params.has("event") && (
        <div className="rounded-lg border p-4 mb-4">
          <p>{t("feedEvidence.linked")}</p>
          <Link
            data-feed-all
            className="underline min-h-[44px] inline-flex items-center"
            href={href({ event: "", cursor: "", period: "", state: "" })}
          >
            {t("feedEvidence.allEvents")}
          </Link>
        </div>
      )}
      ...
      {!feed.error && feed.data?.items.length === 0 && (
        <FeedEmptyState
          more={feed.data.has_more}
          nextHref={
            feed.data.next_cursor
              ? href({ cursor: feed.data.next_cursor })
              : undefined
          }
          filtered={Boolean(
            params.get("state") ||
            (params.get("period") && params.get("period") !== "all"),
          )}
        ...
        {feed.data?.items.map((item) => (
          ...
          >
            <div className="flex flex-wrap items-center gap-2 text-sm">
              ...
              <span>
                {translate(locale, `topics.kind.${item.document_kind}`) ||
                  label(item.document_kind)}
              </span>
              <span>
                {translate(locale, `topics.kind.${item.type}`) ||
                  label(item.type)}
              </span>
            </div>
            ...
            <dl className="text-sm grid gap-2 sm:grid-cols-2 mb-4">
              <div>
                <dt className="muted">{t("feed.detected")}</dt>
                <dd>
              ...
              <div>
                <dt className="muted">{t("feed.officialStatus")}</dt>
                <dd>
            ...
            <FeedInterestBrief
              key={`${item.event_id}:${query}`}
              eventId={item.event_id}
            />
            <MonitorThis kind="event" id={item.event_id} />
            {sourceLink(item.source_url) && (
              <a
                href={sourceLink(item.source_url)}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex min-h-[44px] items-center underline font-semibold"
              >
            ...
            <Link
              data-feed-permalink
              className="underline inline-flex min-h-[44px] items-center ml-4"
              href={href({
                event: item.event_id,
                cursor: "",
                period: "",
            ...
            >
              {t("feedEvidence.openEvent")}
            </Link>
            {item.monitored_documents?.length > 0 && (
              <FeedWatchList
                key={`${item.event_id}:${query}`}
                eventId={item.event_id}
                items={item.monitored_documents}
                nextCursor={item.monitored_documents_next_cursor || null}
              />
            )}
            {item.topic_matches.length > 0 && (
              <FeedTopicList
                key={`${item.event_id}:${query}`}
                eventId={item.event_id}
                items={item.topic_matches}
                nextCursor={item.topic_matches_next_cursor || null}
              />
            )}
            {item.law_impacts.length > 0 && (
              <section className="border-t mt-3 pt-4">
                <h3 className="text-base font-semibold">
                  {t("registry.monitored")}
                </h3>
                <ul className="space-y-4">
                  {item.law_impacts.map((law) => (
                    <li key={law.organization_candidate_id}>
                      <Link
                        href={law.links.timeline}
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Status value={law.severity} />
                      </div>
                      <details className="text-sm mt-2">
                        <summary className="cursor-pointer min-h-[44px]">
                          {t("impact.effect")}
                        </summary>
                        <p>{law.potential_effect}</p>
                        <p>{law.suggested_next_step}</p>
                      </details>
                      <Link
                        href={`/impact?candidate=${encodeURIComponent(law.organization_candidate_id)}`}
                        className="underline min-h-[44px] inline-flex items-center"
                      >
                        {t("feed.review")}
                      </Link>
            ...
            <label className="text-sm block border-t mt-4 pt-4">
              {t("filter.myState")}
              <select
                disabled={busy === item.event_id}
                value={item.read_state}
                className="interest-feed-control block mt-2 min-h-[44px] w-full sm:w-auto"
                onChange={(event) =>
                  void setReadingState(item, event.target.value)
                }
              >
                {states.map((state) => (
                  <option key={state} value={state}>
      ...
      <InboxPageNavigation
        page={
          feed.data
            ? { ...feed.data, total_events: feed.data.items.length }
            : undefined
        }
