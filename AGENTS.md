...
- This checkout belongs to Monitoring v2. Its local `BACKLOG_MONITORING_V2.md` is the sole active implementation backlog: 72 tasks, 63 required and 9 deferred. Its copy on `main` is a published snapshot with a link to this version. Keep hackathon-only tasks and support activity out of it; record any cross-workstream reuse as an explicit receiving task.
- Separate v2 deployment setup is in progress; live routing and branch-triggered updates are not yet verified. MV2-072 is BLOCKED on SMTP setup and dedicated tunnel access; the pinned controller and disabled Windows task are installed. Follow its acceptance criteria and isolate deployment, data, credentials and resources from the main-product/hackathon environment; it does not complete any Pollen Watch feature or gate.
