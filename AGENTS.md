...
- Separate v2 deployment setup is in progress; live routing and branch-triggered updates are not yet verified. MV2-072 is IN PROGRESS: dedicated tunnel/hostname configured and SMTP STARTTLS/authentication verified; bootstrap is underway and the Windows task remains disabled. Follow its acceptance criteria and isolate deployment, data, credentials and resources from the main-product/hackathon environment; it does not complete any Pollen Watch feature or gate.
