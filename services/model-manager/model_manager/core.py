    ...
    def runtime_snapshot(self) -> dict:
        with self.lock:
            ...
            return copy.deepcopy({
                ...
                "quantization": deployment.get("quantization") if available else None,
                "runtime_image": deployment.get("runtime_image") if available else None,
                "hardware_profile": deployment.get("hardware_profile") if available else None,
                "hardware": deployment.get("hardware", {}) if available else {},
    ...
    def start_model(self, model_id: str, profile_name: str | None = None) -> dict:
        ...
        with self.lock:
            ...
            self.state["deployment"] = {
                ...
                "hardware_profile": profile["name"],
                "hardware": {
                    "cuda_devices": copy.deepcopy(self.hardware.get("cuda_devices", [])),
                    "ram_bytes": self.hardware.get("ram_bytes"),
                },
