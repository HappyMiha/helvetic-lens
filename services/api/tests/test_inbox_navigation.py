...
def test_options_remain_available_outside_page_and_limit_without_loading_laws(harness):
    ...
    with service.db.organization_context(other_id), service.db.session() as session:
        assert ImpactInboxReader(other_id, None, settings=service.settings, prompts=service.prompt_settings).law_options(session, selected=ids[-1]) == {"items": [], "has_more": False, "selected": None}
