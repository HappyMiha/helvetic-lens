...
def test_profile_edit_invalidates_history_inbox_and_severity_without_spending_tokens(harness):
    ...
    with service.db.session() as session:
        current, latest, total = ImpactInboxReader._latest_analyses(session, delivery, settings=service.settings, prompts=service.prompt_settings)
...
def test_another_organizations_matching_revision_cannot_validate_a_stale_report(harness):
    ...
    with service.db.session(include_all_organizations=True) as session:
        ...
        session.commit()
    with service.db.session(include_all_organizations=True) as session:
        current, latest, count = ImpactInboxReader._latest_analyses(session, delivery, settings=service.settings, prompts=service.prompt_settings)
        assert current is None and latest.id == saved["id"] and count == 1
        histories, _ = ImpactInboxReader(service.organization_id, None, settings=service.settings, prompts=service.prompt_settings)._page_histories(session, [delivery])
