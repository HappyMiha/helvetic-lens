...
def history_rows(service, delivery_id, count=10_000):
    with service.db.session() as session:
        ...
        base = {
            ...
            "analysis_plan": {"execution": {"profile_revision": 1, "configuration_fingerprint": relation_analysis.configuration_fingerprint(service.settings)}},
...
def test_large_history_reads_only_latest_and_current_payloads(harness):
    ...
    with service.db.session() as session:
        ...
        try:
            current, latest, count = ImpactInboxReader._latest_analyses(session, delivery_id, settings=service.settings)
        finally:
...
def test_empty_legacy_and_current_history_keep_selection_and_tenant_scope(harness, result, status):
    ...
    delivery_id, _ = relation_delivery(harness)
    with service.db.session() as session:
        assert ImpactInboxReader._latest_analyses(session, delivery_id, settings=service.settings) == (None, None, 0)
...
def check_history_index_roundtrip(service, delivery_id):
    ...
    with service.db.session() as session:
        before = ImpactInboxReader._latest_analyses(session, delivery_id, settings=service.settings)
    ...
    with service.db.session() as session:
        after = ImpactInboxReader._latest_analyses(session, delivery_id, settings=service.settings)
