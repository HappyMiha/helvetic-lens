    ...
    *,
    ...
    async def correlated_request_metrics(request, call_next):
        ...
        try:
            ...
            if request.url.path.startswith(("/api/auction-watch", "/api/tender-watch", "/api/commute-watch", "/api/road-watch", "/api/hazard-watch", "/api/trademark-watch")):
    ...
    def interest_feed_topics(event_id: str, request: Request, cursor: str = Query(default="", max_length=4096),
                             limit: int = Query(default=20, ge=1, le=50)):
        identity = request.state.identity
        return service.interest_feed_topics(event_id, identity.user_id if identity else None, cursor=cursor, limit=limit)
    ...
    def impact_inbox_page(
        ...
        watched_law: str = Query(default="", max_length=36),
        state: str = Query(default="", max_length=20),
        candidate: str = Query(default="", max_length=36),
        cursor: str = Query(default="", max_length=4096),
        limit: int = Query(default=50, ge=1, le=50),
    ):
        identity = request.state.identity
        return service.impact_inbox_page(
            ImpactInboxFilters(source=source, severity=severity, item_type=item_type, watched_law=watched_law, state=state, candidate=candidate),
            identity.user_id if identity else None, cursor=cursor, limit=limit,
        )

    @app.get("/api/impact-inbox/law-options")
    ...
    @app.get("/api/integration-logs/briefs/{assessment_id}/attempts")
    def interest_brief_attempts(assessment_id: uuid.UUID, request: Request):
        identity = request.state.identity
        if identity and identity.role != "organization_admin":
            raise DomainError("Organization administrator access is required.", 403, "forbidden")
        with service.db.session() as session:
