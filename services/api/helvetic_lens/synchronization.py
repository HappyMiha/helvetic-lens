...
from .topic_matching import enqueue_live_events
...
DEFAULT_SCHEDULES = (
    ...
    ScheduleDefinition("fedlex", "consultations", 21_600, 600, "complete consultation keyset cycle", 0.2),
    ScheduleDefinition("federal-news", "news-de", 1_800, 180, "fixed window with overlap", 0.2),
    ScheduleDefinition("federal-news", "news-fr", 1_800, 180, "fixed window with overlap", 0.2),
    ScheduleDefinition("federal-news", "news-it", 1_800, 180, "fixed window with overlap", 0.2),
    ScheduleDefinition("federal-news", "news-rm", 3_600, 300, "fixed window with overlap", 0.2),
    ScheduleDefinition("federal-news", "news-en", 3_600, 300, "fixed window with overlap", 0.2),
    ScheduleDefinition("finma-news", "news-de", 3_600, 300, "RSS watermark with two-day overlap", 0.2),
    ScheduleDefinition("finma-news", "news-fr", 3_600, 300, "RSS watermark with two-day overlap", 0.2),
    ScheduleDefinition("finma-news", "news-it", 3_600, 300, "RSS watermark with two-day overlap", 0.2),
    ScheduleDefinition("finma-news", "news-en", 3_600, 300, "RSS watermark with two-day overlap", 0.2),
...
def finish_run(
    ...
    *,
    ...
    result["topic_matches"] = enqueue_live_events(session, events, settings or Settings())
    finished = _aware(now or utcnow())
    run.finished_at = finished
    if run.started_at:
        run.duration_ms = max(0, int((finished - _aware(run.started_at)).total_seconds() * 1000))
