...
def enqueue_after_matching(session, settings, event_ids, trigger):
    ...
    if not settings.interest_brief_auto_enabled:
        return None
    ...
    locale = settings.interest_brief_auto_locale
    trigger_key = fingerprint(trigger)
    key = fingerprint({"version": 1, "events": ids, "trigger": trigger_key, "locale": locale})
    job, reused = jobs.enqueue(session, organization_id=organization_id, job_type=TYPE,
        ...
        payload={"event_ids": ids, "locale": locale, "trigger": trigger_key, "checkpoint": {"position": 0}},
        steps=[("Admit current shared relevance briefs", {})])
    return {"job_id": job.id, "reused": reused, "events": len(ids), "ai_calls": 0}
...
class AdmissionJobs(BriefJobs):
    ...
    async def _execute(self, job_id, worker):
        ...
        def guard(session):
            ...
            if (not self.runner.client.settings.interest_brief_auto_enabled
                    or self.runner.client.settings.interest_brief_auto_locale != payload.get("locale")):
                raise DomainError("Automatic brief policy changed.", 409, "interest_policy_changed")
            return current
        ...
        try:
            ...
            if (set(payload) != {"event_ids", "locale", "trigger", "checkpoint"}
                    ...
                    or job.idempotency_key != "interest-admission:" + fingerprint({"version": 1,
                        "events": ids, "trigger": payload.get("trigger"), "locale": locale})
