...
const server=createServer(async(req,res)=>{
  ...
  try{
    ...
    if(path.startsWith("/api/")||path.startsWith("/__qa/")){
      ...
      if(path==="/__qa/audit"){audits.push(body);await mkdir(resolve(root,"test-results/accessibility"),{recursive:true});await writeFile(resolve(root,"test-results/accessibility/auction-audit.json"),JSON.stringify(audits,null,2));return json({saved:true});}
      if(path==="/__qa/finish"){
        await writeFile(resolve(root,".tmp/auction-browser-requests.json"),JSON.stringify(requests,null,2));
        json({finished:true});server.close();child.kill();return;
      }
      requests.push({path,method:req.method,body});
      if(path==="/api/auth/session")return json({authenticated:true,user:{id:"auction-qa",name:"Auction QA",email:"synthetic@example.invalid",locale:state.locale},organization:{id:"auction-qa-org",name:"Synthetic QA"},role:state.manager?"organization_admin":"viewer",platform_admin:false,onboarding_required:false});
      ...
      if(path.startsWith("/api/auction-watch")){
        ...
        if(req.method!=="GET"&&(body.expected_version!==monitor.version||state.conflict))return json({code:"auction_version_conflict"},409);
