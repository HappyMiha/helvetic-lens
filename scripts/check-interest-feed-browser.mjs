...
try {
  ...
  cdp.on("Fetch.requestPaused", async ({ requestId, request }) => {
    ...
    if (url.pathname === "/api/auth/session") body = { authenticated: true, user: { id: "qa", email: "qa@example.invalid", name: "QA", locale }, organization: { id: "qa-org", name: "Isolated QA" }, role: "viewer" };
      ...
      else body = {items: Array.from({length:20}, (_, i) => ({watch_id:`watch-more-${i}`,name:`Additional monitored document ${i}`,url:`/laws/additional-${i}`})),next_cursor:null};
    }
  ...
  for (locale of ["de-CH", "fr-CH", "it-CH", "rm-CH", "en-CH"]) {
    for (const width of [390, 1440]) {
      ...
      const docMarker = await evaluate(cdp, "window.__feedDocument");
      await evaluate(cdp, `document.querySelector('[data-watch-next]').click()`);
      await waitFor(() => evaluate(cdp, `document.querySelector('[data-feed-watches]').innerText.includes('Synthetic watch page unavailable')`), "Page error missing");
      assert.ok(await evaluate(cdp, `document.querySelector('[data-feed-watches]').innerText.includes('Direct monitored document')`), "Error removed current page");
      await evaluate(cdp, `document.querySelector('[data-watch-next]').click()`);
      await waitFor(() => evaluate(cdp, `document.querySelector('[data-feed-watches]').innerText.includes('Additional monitored document 19')`), "Retry failed");
      assert.equal(await evaluate(cdp, `document.querySelectorAll('[data-feed-watches] li').length`), 20);
      assert.equal(await evaluate(cdp, "window.__feedDocument"), docMarker, "Watch paging reloaded document");
      assert.ok(await evaluate(cdp, `document.documentElement.scrollWidth <= innerWidth + 1`), "Watch page overflow");
