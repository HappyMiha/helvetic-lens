...
import {trademarkEmailFixture} from "./trademark-email-fixture.mjs";
...
const emailRoute=trademarkEmailFixture();
  ...
  try{
    ...
    if(path.startsWith("/api/")||path.startsWith("/__qa/")){
      ...
      if(path.startsWith("/api/trademark-watch")){
        ...
        const email=emailRoute(route,req.method,body,monitor,state);
        if(email)return json(email.value,email.status);
        ...
        if(route==="/deadline-calendars")return json({items:state.deadlineCalendars?[{key:"synthetic-basel",name:"Synthetic Basel calendar",jurisdiction:"Synthetic Basel",covers_from:"2026-01-01",covers_until:"2026-12-31"}]:[]});
        if(route==="/monitors"){
          if(req.method==="POST"){
