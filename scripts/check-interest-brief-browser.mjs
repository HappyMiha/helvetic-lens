...
const root=resolve(import.meta.dirname,".."),audit=new AccessibilityAudit("interest-brief");
...
let cdp,locale="en-CH",status="available",failure=false;const requests=[],exceptions=[];
...
try{
 ...
 for(const width of [390,1440])for(locale of ["de-CH","fr-CH","it-CH","rm-CH","en-CH"]){
  failure=false;status="available";const start=requests.length;
  ...
  failure=true;await click("[data-feed-brief] button");await waitFor(()=>evaluate(cdp,"document.querySelector('[data-feed-brief]').innerText.includes('Synthetic read failure')"),"Read error missing");
  assert.equal(await evaluate(cdp,"!!document.querySelector('[data-brief-result]')"),false,"Error exposed old result");
  failure=false;status="stale";await click("[data-feed-brief] button");
  ...
  for(status of ["not_scheduled","pending","failed","runtime_unverified","not_current","unavailable"]){await click("[data-feed-brief] button");await waitFor(()=>evaluate(cdp,`!!document.querySelector('[data-brief-status=${status}]')`),`Missing ${status}`);assert.equal(await evaluate(cdp,"!!document.querySelector('[data-brief-result]')"),false);}
 ...
 assert.deepEqual(exceptions,[]);audit.finish(20);console.log("Ten five-locale desktop/mobile saved-brief journeys passed; no automatic generation or writes.");
