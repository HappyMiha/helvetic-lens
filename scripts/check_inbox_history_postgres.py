...
def main():
    ...
    with TemporaryDirectory(prefix="helvetic-inbox-history-") as artifacts:
        ...
        with TestClient(app) as client:
            ...
            if args.suite.startswith("digest-topics"):
                from test_digest_topics import (
                    test_one_digest_event_combines_law_and_multiple_topics,
                    test_prepared_topic_evidence_is_rechecked_before_send,
                    test_topic_only_digest_is_grouped_and_uses_saved_evidence_without_inference,
                    test_topic_selection_pages_scope_deduplication_and_empty_continuation,
                    test_topic_severity_source_and_personal_filters_do_not_invent_impact,
                )
                with MonkeyPatch.context() as patch:
                    if args.suite == "digest-topics":
                        test_topic_only_digest_is_grouped_and_uses_saved_evidence_without_inference(harness, patch)
                    elif args.suite == "digest-topics-mixed":
                        test_one_digest_event_combines_law_and_multiple_topics(harness)
                    elif args.suite == "digest-topics-pages":
                        test_topic_selection_pages_scope_deduplication_and_empty_continuation(harness)
                    elif args.suite == "digest-topics-filters":
                        test_topic_severity_source_and_personal_filters_do_not_invent_impact(harness)
                    else:
                        test_prepared_topic_evidence_is_rechecked_before_send(harness, patch, args.suite.removeprefix("digest-topics-"))
                print(f"PostgreSQL: {args.suite} passed; saved evidence only, no real email or inference.")
                return
            ...
            if args.suite.startswith("evidence-revisions-"):
                ...
                return
            if args.suite.startswith("action-"):
                from test_action_history_index import (
                    test_action_index_roundtrip_preserves_decisions_and_cursor,
                    test_deep_action_cursor_seeks_without_scanning_newer_decisions,
                )
                from test_action_history_pages import (
                    test_action_history_explicit_scope,
                    test_action_history_new_decision_and_deleted_boundary,
                    test_complete_action_history_pages_and_compact_summary,
                )
                case = args.suite.removeprefix("action-")
                if case == "index":
                    test_action_index_roundtrip_preserves_decisions_and_cursor(harness)
                elif case in {"seek", "seek-ties"}:
                    test_deep_action_cursor_seeks_without_scanning_newer_decisions(harness, tied=case == "seek-ties")
                elif case == "pages":
                    test_complete_action_history_pages_and_compact_summary(harness)
                elif case == "boundary":
                    test_action_history_new_decision_and_deleted_boundary(harness)
                else:
                    test_action_history_explicit_scope(harness, case)
                print("PostgreSQL:", args.suite, "passed; synthetic decision history only.")
                return
            if args.suite.startswith("summary-"):
                from test_document_summary import (
                    test_current_version_metadata_cannot_follow_a_wrong_reference,
                    test_saved_report_summary_loads_diff_only_for_freshness_and_never_version_body,
                    test_scan_selection_rejects_foreign_or_mismatched_links,
                    test_summary_prefers_scan_comparison_and_uses_deterministic_scan_ties,
                )
                case = args.suite.removeprefix("summary-")
                if case == "report":
                    test_saved_report_summary_loads_diff_only_for_freshness_and_never_version_body(harness)
                elif case == "scan":
                    test_summary_prefers_scan_comparison_and_uses_deterministic_scan_ties(harness)
                elif case.startswith("version-"):
                    test_current_version_metadata_cannot_follow_a_wrong_reference(harness, case.removeprefix("version-"))
                else:
                    case = {"scan-scope": "scan", "wrong-law": "wrong_law"}.get(case, case)
                    test_scan_selection_rejects_foreign_or_mismatched_links(harness, case)
                print("PostgreSQL:", args.suite, "passed; synthetic summary only.")
                return
            if args.suite.startswith("matrix-"):
                from test_matrix_selection import (
                    test_matrix_batches_51_documents_without_per_document_history_queries,
                    test_matrix_explicit_scope_in_privileged_session,
                    test_matrix_invalid_json_results_do_not_displace_saved_object,
                    test_matrix_large_history_loads_only_selected_comparison_and_result,
                    test_matrix_latest_comparison_tie_does_not_borrow_older_report,
                    test_matrix_rechecks_cache_key_when_loading_selected_result,
                )
                case = args.suite.removeprefix("matrix-")
                checks = {
                    "cache": test_matrix_rechecks_cache_key_when_loading_selected_result,
                    "history": test_matrix_large_history_loads_only_selected_comparison_and_result,
                    "json": test_matrix_invalid_json_results_do_not_displace_saved_object,
                    "batches": test_matrix_batches_51_documents_without_per_document_history_queries,
                    "latest": test_matrix_latest_comparison_tie_does_not_borrow_older_report,
                }
                if case in checks:
                    checks[case](harness)
                else:
                    test_matrix_explicit_scope_in_privileged_session(harness, case)
                print("PostgreSQL:", args.suite, "passed; synthetic matrix only.")
                return
