...
const pack = () => ({id: "fedlex-legislation", name: text("Synthetic official legislation"), description: text("Saved official sources"), expected_first_data: text("Data arrives after a successful scheduled sync."),
...
try {
  ...
  cdp.on("Fetch.requestPaused", async ({requestId, request}) => {
    ...
    else if (url.pathname === "/api/source-packs") {await sleep(100); body = {starter: {id: "starter", name: text("Synthetic starter package"), description: text("Review before activation."), expected_first_data: text("No complete coverage promised."), state: "inactive"}, items: [pack()]};}
  ...
  for (const language of ["de-CH", "fr-CH", "it-CH", "rm-CH", "en-CH"]) for (const width of [390,1280,1440]) for (const permission of ["organization_admin", "viewer"]) {
    locale = language; role = permission; user = `${language}-${width}-${permission}`; packEnabled = false; packPending = false;
