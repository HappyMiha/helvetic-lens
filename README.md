...
> **Public-beta direction:** Local AI becomes the default and cloud providers become explicit optional adapters. The target remains one physical i7/32 GB/two-GTX-1080 server and one Compose deployment. Durable jobs, managed quantized models, account sessions, organization invitations, and enforced administrator/viewer roles are implemented; complete German/French/Italian/Romansh/English localization, three official-source connectors, a time-grouped registry, and an impact inbox follow in the backlog. See the [architecture decision](docs/ARCHITECTURE.md) and `HL-029`–`HL-049` plus `HL-057` in [BACKLOG.md](BACKLOG.md).
