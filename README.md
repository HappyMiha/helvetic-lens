...
1. Choose **Infomaniak AI** or **Other OpenAI-compatible API**. Infomaniak needs its numeric Product ID; the custom option needs the API base URL, including /v1 if required. Do not append /chat/completions; RegWatch adds it.
...
3. Adjust the request timeout, evidence warning threshold, maximum completion length, temperature, Top P, presence penalty, reasoning effort, and optional JSON mode. RegWatch keeps `stream=false` and `n=1` fixed so structured answers and citations can be validated consistently.
