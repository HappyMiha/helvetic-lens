...
> **Public-beta direction:** Local AI becomes the default and cloud providers become explicit optional adapters. The target remains one physical i7/32 GB/two-GTX-1080 server and one Compose deployment. Durable PostgreSQL jobs and Redis/Celery queues are implemented; managed quantized models, organization workspaces, login, complete German/French/Italian/Romansh/English localization, three official-source connectors, a time-grouped registry, and an impact inbox follow in the backlog. See the [architecture decision](docs/ARCHITECTURE.md) and `HL-029`–`HL-049` plus `HL-057` in [BACKLOG.md](BACKLOG.md).
...
Open **Local models** to inspect the host, accept a model license for an exact immutable revision, and download, pause, resume, verify, start, stop, or remove an allowlisted Apertus GGUF. Transfers run in the durable maintenance queue, use `.part` files, check free space and SHA-256, and activate atomically. The private manager owns the pinned CUDA llama.cpp runtime and model volume; the public API receives neither a Docker socket nor an arbitrary model URL/command. Starting a model selects its local OpenAI-compatible ID for new application requests while preserving any saved cloud credential as an optional fallback configuration. The current catalogue includes a small 1.5B Q4 diagnostic profile for the GTX 1070 and an 8B Q4 candidate that remains subject to the HL-032 target-host benchmark.

For a terminal smoke test, run `./scripts/local_apertus.ps1`. It starts the manager, adopts a compatible cached artifact when available (otherwise resumes its pinned download), verifies its hash, starts it on GPU, and checks an actual structured `{"status":"ok"}` response. Helvetic Lens uses `http://127.0.0.1:12435/v1` on the host and `http://model-manager:8080/v1` inside Compose. Model-manager control is loopback-only at port 12436 on the development host and private inside Compose.
...
The current Compose stack has PostgreSQL-backed durable jobs and Redis/Celery workers, but it still binds to loopback and has no user authentication, Caddy/TLS, organization roles, private managed inference endpoint, backup rehearsal, or measured 100-user capacity result. Do not expose it publicly yet; see [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
| Queue and scheduler          | **Redis + Celery/Beat**                        | Implemented for durable scan and AI jobs with six queues, persisted steps, leases, retries, cancellation, recovery, and an Activity view. Scheduled connector work expands in HL-038–HL-042.                     |
