...
**License: [Apache License 2.0](LICENSE)** · OSI-approved permissive license.
Use, copy, and distribution are allowed under Apache-2.0 terms, including
commercial use. See the [licensing guide](docs/LICENSING.md) and
...
> **Project status:** The model-enabled MVP works end to end: source connections, direct URLs, imports, immutable history, live scans, saved comparisons, visual evidence, persisted Apertus settings, impact analysis, cited questions, saved AI history, editable prompts, inspectable integration logs, personal web/email digests, and controlled removal of sources or monitored documents. Scans and AI work now use durable PostgreSQL jobs with Redis/Celery workers, persisted stages, cancellation, retry, result links, and restart recovery. The current triage layer keeps the complete exact comparison for inspection, separates likely material wording from formatting and renumbering noise, blocks obvious document mismatches, and gives AI a fixed-size evidence dossier instead of starting one request per passage. Official coverage is published through a [versioned source capability catalogue](docs/SOURCE_CAPABILITIES.md), while organizations activate it through [source packs](docs/SOURCE_PACKS.md) without duplicating the shared corpus. See [verification notes](docs/VERIFICATION.md), [digest contract](docs/DIGESTS.md), and the [AI triage contract](docs/AI_TRIAGE.md).
...
**Active development plan:** [Helvetic Lens Monitoring v2](BACKLOG_MONITORING_V2.md) is the single backlog for all further work. It plans nine active personal/business monitoring templates over a shared change engine; these v2 capabilities are not yet implemented. Pollen Watch is the first complete delivery, with dedicated C5 source/platform tasks and an independent real-user-testing readiness gate (MV2-069/070/030/031/071). Customs rates (C4) are a possible future feature, excluded from current development and v2.0 acceptance. See the [architecture](docs/monitoring-v2/ARCHITECTURE.md), [requirements traceability](docs/monitoring-v2/REQUIREMENTS_TRACEABILITY.md), [source feasibility](docs/monitoring-v2/SOURCE_FEASIBILITY.md), and [legacy continuation](docs/monitoring-v2/LEGACY_DISPOSITION.md). The frozen [Hackathon MVP release](https://github.com/HappyMiha/helvetic-lens/releases/tag/v1.0.0-hackathon-mvp) remains available. [BACKLOG.md](BACKLOG.md) is the entry point; the original backlog is preserved as [history](BACKLOG_V1_ARCHIVE.md).
...
The application uses managed local quantized Apertus by default and can optionally use Infomaniak or another compatible endpoint. Cloud use is an explicit organization-level opt-in and is never a silent fallback. The intended larger model is [Apertus v1.5 8B](https://huggingface.co/swiss-ai/Apertus-v1.5-8B), subject to the measured GTX 1080 compatibility gate. The served ID depends on the runtime/provider. Model weights are not bundled and no alternate model is silently substituted.
...
A runtime or structured-JSON benchmark proves that a model can be served within measured limits. It does not establish that its explanations are accurate, relevant, fluent, or useful for regulatory work. Those claims require the independent answer-quality, localization, and user-review evidence tracked in the backlog; the measured 1.5B development profile is not a general legal-quality endorsement.
...
Edit **Company profile** on the **Organization** page to supply business context; Settings links to that same editor. Impact analysis and Ask Apertus need actual model responses with working evidence links for acceptance. When disconnected or unavailable, the UI says so and keeps the diff usable.
...
| **Apertus impact analysis**       | Preserve the complete exact comparison, exclude detected layout/renumbering noise from inference, and analyse a fixed-budget dossier of likely material changes. Return a validated report with old/new legal units, exact evidence, organization applicability, explicit unknown dates, separate evidence strength, and up to five deduplicated structured review suggestions. Zero actions is a valid result. See [the impact-report contract](docs/IMPACT_REPORT.md).                                                     |
| **Decision-ready review**         | Open a deterministic material-change overview immediately, follow persisted background inference without blocking the diff, inspect one current report with full provenance, and accept, assign, schedule, dismiss, or mark a suggested action not applicable with organization-scoped decision history. See [comparison review](docs/COMPARISON_REVIEW.md). |
...
| **Ask Apertus with citations**    | Ask questions about the selected comparison through seven explicit intents. Canonical change/impact/action questions reuse the current validated report where possible; specific articles use only the unit and neighbours; whole-document questions disclose full or targeted scope. Vague and off-topic input returns useful choices with zero model calls. Follow-ups carry validated prior answers and citations. See [Ask routing](docs/ASK_ROUTING.md). |
...
The development Compose stack binds to loopback. The separate [production single-server deployment](docs/PRODUCTION_DEPLOYMENT.md) adds fail-closed configuration validation, startup migration, Caddy TLS on ports 80/443, private data/model networks, readiness ordering, fixed image digests, restart policies, and bounded container logs. The checked-in [recovery and 100-user capacity gate](docs/CAPACITY_GATE.md) seeds several organizations, drives concurrent reads/scans/connector/AI work, restarts every runtime service, samples host/container/GPU resources, and combines the target inference plus backup/restore evidence. The physical dual-GTX-1080 result remains a release gate, so do not open public registration yet; see [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
| ---------------------------- | ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Web app                      | **Next.js**                                    | Implemented with registry, topics, source packs, impact inbox, login, organization, evidence, comparison, model, prompt, and diagnostic views. Five-language controls and checks are implemented; native-language review remains open. |
| Styling and components       | **Tailwind CSS + shadcn/ui**                   | Shared components and responsive layouts are implemented. Measured usability, visual consistency, and accessibility remain explicit product acceptance work. |
...
| Queue and scheduler          | **Redis + Celery/Beat**                        | Durable scan, AI, and scheduled connector work is implemented with six queues, persisted steps, leases, retries, cancellation, recovery, and an Activity view. |
...
- [x] **M6 implementation:** durable jobs, managed local models, shared corpus, login, organizations, enforced roles, and the five-locale catalogue/error/date foundation are implemented (`HL-029`–`HL-035`, foundation of `HL-057`).
- [ ] **M6 acceptance:** complete the physical dual-GTX-1080 benchmark in `HL-032`; native-language review remains part of `HL-057`.
- [x] **M7:** normalized registry plus bounded incremental Fedlex, Parliament, Federal Supreme Court, scheduling, and official notices are implemented (`HL-036`–`HL-043`). Individual streams retain their verified or partial coverage states.
- [x] **M8:** explainable relation candidates, local cited impact analysis, and the grouped organization impact inbox are implemented (`HL-044`–`HL-046`).
- [ ] **M9 acceptance:** administration, production deployment code, and five-language UI/AI history are implemented; finish native-language review, target-host operations/recovery rehearsal, and the measured 100-user gate (`HL-047`–`HL-049`, `HL-057`).
- [ ] **Daily-use loop:** complete stateful onboarding, the unified law/topic feed, persisted relevance briefs, contextual monitoring, and topic notifications/digests (`HL-073`, `HL-076`–`HL-079`, `HL-089`). Shell, responsive comparison, asynchronous Ask, source packs, and topic persistence/matching already provide the foundation.
- [ ] Add graph review or multi-host deployment only after their entry conditions; broader news, measured pgvector recall, personal digests, identity refinements, and a second federal court are implemented (`HL-050`–`HL-056`).
