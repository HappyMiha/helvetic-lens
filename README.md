...
| **Official connectors**           | Run Fedlex, Parliament, and court adapters through the same versioned discovery, metadata, expression, artifact, relation, and health boundary. The implemented Fedlex catalogue adds overlapping DE/FR/IT RSS discovery, bounded `cc`/`oc`/`fga` reconciliation, language expressions, dated manifestations, official JOLux relations, safe page checkpoints, and idempotent merge with direct ELI monitoring. Source drift is degraded without inventing unchanged or repealed records. |
...
Public beta uses three bounded official connectors rather than patches for individual URLs. Fedlex is implemented; Parliament and court adapters remain on the roadmap:
