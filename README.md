...
Native Fedlex resolution currently recognises Classified Compilation (`cc`), Official Compilation (`oc`), and Federal Gazette (`fga`) ELI URLs ending in `de`, `fr`, `it`, `rm`, or `en`. The requested language must actually exist. Other JavaScript routes and search pages still require a supported direct document URL. If Fedlex metadata or its publication file is unavailable, the preview/scan fails explicitly and the last good snapshot remains unchanged. A successful extraction does not establish complete legal coverage.
