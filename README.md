...
> **Public-beta direction:** Local AI becomes the default and cloud providers become explicit optional adapters. The target remains one physical i7/32 GB/two-GTX-1080 server and one Compose deployment. Durable jobs, managed quantized models, account sessions, enforced organization roles, the normalized regulatory corpus, time-grouped registry, three official-source connectors, persisted scheduled synchronization, and cited fixed-budget analysis of possible effects on monitored laws are implemented; complete German/French/Italian/Romansh/English localization and the impact inbox follow in the backlog. See the [architecture decision](docs/ARCHITECTURE.md), [relation-impact contract](docs/RELATION_IMPACT_ANALYSIS.md), and `HL-029`–`HL-049` plus `HL-057` in [BACKLOG.md](BACKLOG.md).
...
- [ ] **M8:** explainable relation candidates and local cited impact analysis are implemented; the organization impact inbox remains (`HL-044`–`HL-046`).
