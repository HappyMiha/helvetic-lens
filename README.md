...
| **Official connectors**           | Run Fedlex, Parliament, and court adapters through the same versioned discovery, metadata, expression, artifact, relation, and health boundary. Fedlex adds overlapping RSS and bounded ELI/JOLux reconciliation. The Parliament adapter adds complete catalogue, recent, and known-active streams with actual language availability, status-only lifecycle events, immutable source text/documents, and exact identifier candidates. Source drift is degraded without inventing unchanged, enacted, or repealed records. |
...
Public beta uses three bounded official connectors rather than patches for individual URLs. Fedlex and Parliament are implemented; the court adapter remains on the roadmap:
