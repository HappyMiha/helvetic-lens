...
| **Official connectors**           | Run Fedlex, Parliament, and Federal Supreme Court adapters through the same versioned discovery, metadata, expression, artifact, relation, and health boundary. Fedlex adds overlapping RSS and bounded ELI/JOLux reconciliation. Parliament adds complete catalogue, recent, and known-active streams. The court adapter adds overlapping latest/date discovery, bounded two-year reconciliation, immutable official HTML, separate decision/publication dates, and grouped exact legal references. Source drift is degraded without inventing legal state. |
...
| Court decisions   | Implemented Swiss Federal Supreme Court overlapping latest/date index and bounded current/previous-year date reconciliation, followed by official decision HTML | The court's published sitemap contains website pages rather than decision records. The adapter respects the two-second crawl delay, separates decision date from insertion date, and does not claim all Swiss courts. |
