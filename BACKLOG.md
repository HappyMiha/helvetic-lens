...
**6 September 2026 follow-up — first recorded personal actions:** topic/document-watch creation and explicit personal notification preferences now retain their first action dates in the same transaction as the action, scoped to the actual user and organization. Both evidence viewers record displayed non-sample text only after visible-viewport intersection, with server-side access and saved-text checks; passive GET/prefetch/download, missing targets and metadata-only pages do not count. The guide shows three recorded dates, preserves them across shared edits and does not infer older activity or human comprehension. PostgreSQL uniqueness/concurrency and additive migration checks pass; five-language mobile/desktop browser tests cover the display boundary. See [personal onboarding](docs/PERSONAL_ONBOARDING.md).

**6 September 2026 follow-up — explicit personal source choice:** the Sources package section summarizes enabled/disabled packages and offers a personal save after reviewing descriptions and limitations. It preserves the exact catalogue/definition revisions and subscription flags, first/latest review dates, supports invited viewers without shared writes and surfaces a stale review after source-selection changes. Outdated submissions return a recoverable conflict; the guide links back to review. Empty package selection has an explicit warning and does not disable directly watched documents. No activation, request, synchronization, mail or AI occurs on review. This is an acknowledged selection, not proof of comprehension or fresh coverage.

**Still open:** recovery integration beyond Today and independent end-to-end coverage proof before claiming a genuinely quiet period; deeper first-value integration; independent five-language/user observation proving unassisted first interest plus evidence in under five minutes. Intent selection and organization row availability are not evidence of first value. This slice does not close HL-073.
...
**6 September 2026 — private history and explicit deletion:** Added **My Marvin conversations** in workspace navigation and the chat privacy notice. The five-language desktop/mobile page reads metadata in bounded pages, then the selected conversation's saved messages, draft and cited-Ask handoffs, without initializing Marvin or invoking AI. It states personal versus shared visibility, the existing 40-message/20-handoff retention limit and deletion boundaries. Viewers can read/delete their own history with CSRF enforcement; even organization administrators cannot read a colleague's conversation. Deleting a personal record preserves shared answers, monitors and saved evidence; an already running reply cannot recreate the deleted record. Identity changes discard old private page state, and same-origin open Marvin tabs receive an identifier-only deletion notice to detach and clear the matching tab draft. See [actual behaviour and limits](docs/MARVIN_PRIVACY.md) and `docs/VERIFICATION.md`. **HL-083 remains IN PROGRESS:** full cited intent execution, explicit record-context selection, durable server cancellation/recovery and measured useful-task success are still open; no comprehensive backup/log erasure, timed retention or cross-device revocation is claimed.

**6 September 2026 — paused and detached context lifecycle:** Fixed the disabled-companion bootstrap observed during document-history QA. Explicit Pause/Enable controls now stop Marvin's new context/conversation/runtime requests and job subscription; the paused return control is in the top bar rather than over page actions. Context detachment persists across navigation/reload/pause until explicit reattachment. Old context requests and draft timers are cancelled, late replies cannot restore/speak old private messages, identity changes remount personal state, and job-cache/tab-draft keys include user and organization. Existing unowned legacy tab drafts are neither imported nor deleted. Loading an unchanged saved draft does not redundantly PATCH it. See [privacy and cancellation boundaries](docs/MARVIN_PRIVACY.md) and `docs/VERIFICATION.md`. **Still open:** server-side durable chat cancellation/recovery, broader intent and retention/deletion UX, explicit record-context selection and useful-task metrics. No claim that browser cancellation revokes accepted server work or that every enabled page visit is write-free. HL-083 remains **IN PROGRESS**.
...
**Implemented 6 September 2026 (capability-bound execution):** Production comparison Ask/Impact now select evidence or explanation adapters from a verified model/task/language profile instead of provider labels. Reviewed input/output/safety/context limits govern measured allocation and every generation/synthesis/repair; profile or review changes invalidate current cache/report/matrix/job reuse while preserving historical records. Mid-request revocation stops acceptance. Integration settings expose installed profile metadata and selection in five languages under existing permissions; the shipped registry stays empty and no real model is promoted. Current same-language report reuse remains a cited zero-call operation with source provenance, not a new Ask approval. **Validation:** 396 combined API/manager tests, production build and 305 full-document browser checkpoints pass; incomplete accessibility findings remain recorded. See [execution contract](docs/AI_CAPABILITY_PROFILES.md) and [verification](docs/VERIFICATION.md). **Remaining:** independent HL-093 explanation/domain evaluation and target-hardware readiness, candidate relevance/packing improvements and separate relation-assessment policy. This slice does not claim legal quality, native-language acceptance or 100-user capacity.

**Implemented 6 September 2026 (measured evidence allocation):** The local selected-evidence adapter now tries the complete candidate batch against native full-prompt counts, then reduces whole change groups and exact saved-text windows within at most eight count probes. Earlier/current pairs and original citation numbers are preserved; omitted rows cannot pass schema/server validation. Ask/Impact history and completed plans record the executed selection and truthful partial coverage, while the full persisted comparison remains unchanged. Count failures cannot cause an unmeasured fallback; the final gateway guard still checks generation and the one same-evidence repair. Cache identity distinguishes the new planner/protocol from older unmeasured answers. **Validation:** real persisted API and client → ASGI gateway scenarios plus a count-only five-language check on the existing native Apertus runtime; full results in [verification](docs/VERIFICATION.md). **Remaining:** capability-approved adapter/task/language budgets, approval freshness and independent explanatory evaluation. This conservative halving algorithm is not optimal packing or verified relevance; preliminary character planning and repair overhead remain explicit limits. No model promotion, production deployment or target-hardware capacity claim. See [allocation contract](docs/LOCAL_INFERENCE.md#measured-evidence-allocation--6-september-2026).

**Implemented 6 September 2026 (complete prompt token guard):** Each local gateway chat request now measures the complete rendered input with the reserved llama.cpp runner before generation and checks actual per-slot context against positive output and safety reserves. Oversized/unverifiable inputs fail once without decoding or silent truncation; counting, generation, retries and repair retain the deployment protections. A count-only route is available for subsequent planner integration. Ask/Impact validate measurement identity and preserve counts separately from usage in history, completed plans and integration logs. **283 combined regressions pass; an isolated offline CPU test on the pinned real runtime matches all six native prompt counts to reported usage (five languages and repair) and counts oversized evidence without generation.** **Remaining:** capability-driven modes and approved-token evidence allocation, approval-decision freshness and independent explanatory evaluation. Existing character-based planning can still select an input rejected by the final guard; no model is promoted and no production deployment or target-GPU capacity gate is claimed. See [contract and limits](docs/LOCAL_INFERENCE.md#complete-prompt-token-guard--6-september-2026).

**Implemented 6 September 2026 (runtime-aware cache/history slice):** Local Ask/Impact cache keys, completed-job reuse, current report selection, comparison/law summaries and matrix freshness share the observed runtime identity. Identical fully identified restarts reuse answers; partially identified models reuse only within the same deployment. Changed/unavailable runtimes cannot silently make an older bound report current. Workers record their actual execution scope; jobs completed after a model change in the queue cannot poison the original enqueue key. Failed Ask attempts remain immutable when retried. Metadata probes have a wall-clock deadline; historical reads need no live model. Cache/stale guidance is corrected in all five locales. **237 combined tests, ten isolated PostgreSQL scenarios, production build and 125 browser accessibility checkpoints pass.** **Remaining:** capability-approved mode/adapter selection, approval-decision freshness, measured prompt token budgets and independent explanatory evaluation; relation-assessment cache policy remains separate. No model promotion or production deployment. See [runtime cache contract](docs/LOCAL_INFERENCE.md#runtime-aware-cache-and-history--6-september-2026).

**Implemented 6 September 2026 (analysis execution-binding slice):** Each new local Impact/Ask inference trace captures one typed runtime snapshot before document transmission and uses its pin through concurrent batches, synthesis, transport retries and JSON repair. Missing/inconsistent runtime data prevents generation; a stopped/replaced deployment or wrong response pin cannot become an accepted answer. Captured launch/hardware metadata and stable immutable identity are saved in history and completed-plan metadata instead of probing whatever model is running after completion. **271 combined regressions pass**, including real analysis-client → ASGI gateway wiring with only the runner mocked and real persisted Ask/Impact success/failure paths. **Remaining:** capability-based routing, complete prompt token measurement, coordinated runtime-aware cache/job/report/matrix freshness and independent explanatory evaluation. Existing cache reuse is not yet migrated; no model was promoted or production deployed. See [execution binding and limits](docs/LOCAL_INFERENCE.md#analysis-execution-binding--6-september-2026).

**Implemented 6 September 2026 (local deployment-binding slice):** The manager re-verifies the installed GGUF at launch, records template/executable inputs and exposes a versioned active-runtime snapshot. The gateway checks an optional deployment pin in the queue and immediately before inference, rejects a wrong served-model ID, prevents manager-driven replacement during the request, and releases reservations on success/error/cancellation. Restarting even the same model invalidates an earlier pin; cancelled/timed-out waiters cannot block the queue. Marvin's local client now checks the selected model/revision/artifact against that snapshot, sends its pin and rejects an unbound/mismatched response. **149 combined tests pass**, including actual client → ASGI gateway wiring with only the runner mocked. **Remaining:** Impact/Ask must carry the same decision/binding through plans, batches/retries/synthesis/cache/history and measure full prompt tokens. Image provenance remains a trusted launch assertion, not independent daemon attestation; no model/task/locale has been promoted and no hardware quality/performance gate was claimed. See [runtime binding and deployment compatibility](docs/LOCAL_INFERENCE.md#deployment-binding--6-september-2026).

**Implemented 6 September 2026 (approval-contract slice):** Added a strict versioned capability registry, checksum-verified per-task/per-locale review records, exact model/artifact/tokenizer/template/runtime/hardware identity binding, bounded token budgets and a provider-independent resolver with deterministic freshness fingerprints. Unselected, unknown, candidate, revoked, identity-mismatched and unreviewed-language/task profiles resolve to selected evidence; no fallback model/provider is chosen. A read-only checker distinguishes valid registry integrity from actual approval availability and never claims semantic validation. **63 synthetic contract/CLI tests pass. The shipped registry approves no model. This is infrastructure, not completed runtime routing:** observed runner identity, planner/wire-adapter/synthesis/cache/history integration, measured request tokens and real HL-093 explanatory evaluations remain open. The existing structured-output benchmark does not promote a profile. See [capability contract and remaining integration](docs/AI_CAPABILITY_PROFILES.md).
...
**Implemented 6 September 2026 (decision-draft slice):** Approved model/task/locale execution now produces report v5 with separate cited change explanations, actual organization activities and applicability conditions, official-status interpretation, explicit action-review state, concrete recommendations and uncertainties. Paired admitted version evidence is retained in synthesis. Nested references, exact obligation anchors, activity IDs and review-state consistency receive validation and one repair within existing call/token guards. Actions sharing an obligation anchor/activity/type merge without invented owners or deadlines. Partial/clipped evidence or unexplained material changes cannot establish global `no_action_now`. Current/history UI distinguishes interpretations, saved wording, legacy reports and unreviewed empty action lists in five locales; Ask reuses conditions and rationale without another call. Existing v2/v3/v4 history is not rewritten. **Remaining:** legal date meanings, relative anchors, semantic equivalence across different source anchors, independently reviewed applicability/status/action accuracy and native-language/target-model acceptance. The shipped approval registry stays empty; synthetic tests are not real model promotion. See [decision contract and limits](docs/IMPACT_REPORT.md#structured-decision-drafts-hl-092-6-september-2026).
...
**Implemented 6 September 2026 (offline matching evaluation):** A versioned package/label/prediction contract binds exact inputs and UTF-8 source snapshots by hash, separates development/held-out families, preserves two independent reviewer votes and distinct adjudication, and retains missing/unreviewed outcomes. A CLI runs the actual production topic scorer in isolated memory without gold labels, external DB/network/model access or application writes. Evaluation publishes real denominators, false-positive/negative IDs, five-language/source/feature strata, disagreement records and separate readiness/target failures. Missing denominators stay unmeasured; synthetic packages and incomplete reviews cannot pass readiness. The checked-in walkthrough intentionally has no independent labels and cannot approve explanatory models. The older title benchmark no longer claims measured evidence-policy compliance or enables embeddings. **Remaining:** actual independently reviewed 200-pair source corpus, frozen real baseline findings, 30 comparison/Ask/report scenarios and separate entailment/action/language assessments, reviewer authentication/attestation checks and target-user evidence. No self-generated gold or profile promotion is claimed. See [evaluation protocol and commands](docs/SEMANTIC_MATCHING_EVALUATION.md).
...
**Implemented 5 September 2026 (draft switch/reload protection):** The editor now distinguishes unsaved changes from its loaded/reset baseline. New-topic, cancel-edit and other-topic actions ask before discarding changes and return to the retained form when cancelled. Reverted/unchanged plans do not prompt; successful saves clear the baseline. Browser reload/unload raises the native warning. The required five-language browser journeys pass 40 discard decisions plus native reload cancellation, preserving preview/revision behavior. **Still open:** client-side route/history guards, crash/mobile termination recovery and the remaining HL-095 criteria; this is not persistent draft storage.

**Implemented 5 September 2026 (everyday registry filters):** Monitoring and Discover now lead with labelled search, Today/Yesterday/last 7/30-day Zurich detection-date presets and impact/monitoring/read filters. Authority/source/kind/language/lifecycle/health/exact dates are disclosed on demand; advanced deep links remain visible. Applied filters have individually removable chips, clear preserves the locale/current route, URL Back restores choices, and filtered empty results offer direct recovery. Fixed options/group headings are localized in all five languages, and mobile row dates no longer squeeze long titles. Six calendar tests, four existing registry API tests, production build and **20 required localized mobile/desktop browser journeys** pass with synthetic intercepted APIs. **Still open:** relevance/feed controls, evidence-return position, topic navigation/crash recovery, full source-health/identity recovery and independent mobile/usability/native-language gates. HL-095 stays **IN PROGRESS**. See [registry filter contract](docs/REGISTRY_FILTERS.md).

**Implemented 5 September 2026 (tab draft recovery):** Meaningful unsaved topic edits survive same-tab navigation/reload through scoped session storage, with explicit Restore/Discard above the editor in five languages. Recovery retains the original revision, editable baseline, AI-draft attribution and activation retry key, but requires a fresh preview before any explicit save. Successful save/revert/discard clears the draft; 24-hour expiration, size/shape validation and visible storage-failure guidance apply. The editor remounts for a different user/organization. Final production build, 13 storage/parser cases, six existing topic API tests and ten required localized mobile/desktop browser journeys pass, including real reload, route/Back, same-key retry, denied storage and account isolation. This is **not** guaranteed crash/mobile-termination preservation, cross-device synchronization or durable backup; those limits are stated in the UI/contract. Complete route warnings, independent usability/mobile/native review and the other HL-095 criteria remain open.

**Implemented 6 September 2026 (registry evidence return):** Same-tab departures from a row retain the scoped result URL, filters/cursor and exact originating link for 30 minutes. Both saved-evidence viewers offer a matching Back to your results link; explicit return and browser Back restore visible focus even when the row is taller than the viewport. Missing rows produce a notice without changing filters; markers are consumed once and cannot restore across users/organizations or arbitrary routes. Monitoring/Discover now use current-page navigation links rather than false tab semantics. Seven helper cases, 20 long-list round trips in five languages, existing filter/evidence regressions and build evidence are recorded in `docs/VERIFICATION.md`. Storage denial falls back to native navigation; no server history, API mutation or production deployment. Remaining HL-095/097 acceptance is unchanged outside this scoped navigation behavior.

**Implemented 6 September 2026 (registry failure recovery):** Initial failures and failed refreshes now have distinct, localized recovery states. Retry preserves the exact request; optional first-page and clear-filter actions are explicit. Cached rows remain inspectable and a cached empty response is not presented as current proof of no developments. The result-load timestamp is separate from source freshness. Row kind/lifecycle/language/health labels are localized, and recorded event health is explicitly historical rather than a live connector claim. Ten additional mobile/desktop recovery journeys plus prior registry/return tests and the build are recorded in `docs/VERIFICATION.md`. Current source-health/coverage, identity-mismatch repair and independent usability/native-language checks remain open; no collection, AI call or production change.
...
**Implemented 6 September 2026 (focused saved-word excerpts):** The first exact change now shows bounded context around its actual saved changed fragment, including additions/deletions near the end of a long article. Saved word parts must reconstruct and align with the original passages; otherwise an explicit plain-text fallback avoids false highlighting. Ellipses, the fragment count and full-evidence navigation disclose the preview limits. Eight helper regressions and the populated five-language browser fixture cover the late 10-to-30-day example. No new inference, re-diff, evidence rewrite or claim of legal significance. Full HL-096 acceptance remains open.
...
**Implemented 6 September 2026 (saved legal-unit labels):** Group headings and search now reuse saved before/after legal-unit hierarchy with exact-side passage attachment checks. Anonymous positions never become article numbers; missing labels keep neutral group names. Repeated labels deduplicate, chapter context disambiguates equal article numbers, renumbering remains two-sided and every label remains searchable beyond the three-label display preview. Five-language copy explicitly says these are extraction labels requiring evidence verification. Unit and populated browser checks are recorded in `docs/VERIFICATION.md`. Independent label correctness, broader design/density consolidation and user acceptance remain open; no data rewrite or inference.
...
**Implemented 6 September 2026 (semantic structure and named navigation):** Corrected standalone/nested monitoring-choice heading levels, distinct localized top/bottom event-pagination names and duplicate page-count announcements. The deterministic change overview now exposes its label through a supported group role. Browser checks inspect actual accessibility-tree names/roles and preserve both heading hierarchies; inbox coverage expands to all five locales at four widths. The combined gate now rejects all reported axe violations and unresolved prohibited ARIA attributes across 265 required checkpoints. See [semantic contract](docs/NAVIGATION_ACCESSIBILITY.md#semantic-structure-and-names--6-september-2026) and verification. **Remaining:** incomplete contrast/manual checks, broader error/dialog/route coverage, physical devices, screen readers, native-language and Firefox/Safari acceptance. HL-097 remains **IN PROGRESS**.

**Implemented 6 September 2026 (mandatory full-document accessibility scans):** Added pinned local axe-core checks to the required Topics, comparison/AI, original-evidence, inbox, onboarding and Marvin production-browser journeys. `npm run check:accessibility` requires all six suites and 249 named visible-fixture checkpoints; critical/serious failures block success without excluded rules or elements. Full findings and incomplete checks remain inspectable. Corrected 28 legacy foreground rules and removed the shared button's disabled-to-enabled opacity transition. See [scope and limits](docs/NAVIGATION_ACCESSIBILITY.md#required-automated-accessibility-journeys--6-september-2026) and the verification record. **Remaining:** nonblocking findings/incomplete manual checks, broader state/route and all-language inbox coverage, real screen-reader/device, Firefox/Safari and independent usability acceptance. HL-097 remains **IN PROGRESS**.

**Implemented 6 September 2026 (Marvin → cited Ask handoff):** Transferring a comparison question now opens and focuses Ask immediately; a slow/unavailable personal-history save no longer blocks this local action. The save has a ten-second request deadline, no automatic retry, and stale responses cannot overwrite a later handoff or changed context. Mandatory production-browser coverage exercises 40 actual handoffs across five locales and four widths, including held history responses, exact multiline/2,000-character drafts, close/reopen persistence, released scroll locks, restored opener and no implicit inference. Existing comparison/evidence and Marvin monitoring journeys remain required. See [handoff contract](docs/NAVIGATION_ACCESSIBILITY.md#marvin-to-cited-ask--6-september-2026). **Remaining:** broader cross-feature/whole-route accessibility, physical-device, assistive-technology and independent usability acceptance; HL-097 is still **IN PROGRESS**.
...
**Implemented 5 September 2026 (comparison overlay focus slice):** Below/equal 1350 px the comparison companion is now a native modal dialog, with browser-managed background inertness, forward/reverse keyboard containment, Escape/backdrop/close dismissal and opener-focus restoration. Main/document scrolling is restored after dismissal; citation jumps close the panel and focus the matching saved passage. Desktop remains nonmodal, and one mounted subtree preserves the unsent question and job state across close/reopen and breakpoint changes. Close also clears the obsolete task URL. `npm run check:comparison:browser` requires a populated synthetic comparison/report/citation fixture and checks all five locales at 390/768/1024 px plus transitions to 1440 px; it cannot silently skip a missing comparison. Production build, 15 browser journeys and 8,060 contrast samples pass. **Remaining:** global navigation/Marvin and nested-overlay journeys, skip-to-main, other required routes, automated whole-journey accessibility findings, manual assistive technology/real mobile input and Firefox/Safari review. HL-097 remains in progress; this is not a complete accessibility certification.

**Implemented 5 September 2026 (mobile navigation focus slice):** Mobile More now uses the shared modal dialog with a localized name, keyboard containment, background scroll/accessibility isolation and opener-focus restoration. The first Escape closes an expanded workspace selector; the next closes navigation. Desktop resize closes the mobile overlay and focuses visible main content. A localized first-stop skip link reaches the actual content, and Marvin/background navigation cannot cover the open menu. `npm run check:shell:browser` passes 30 required populated journeys (five locales × three roles × two widths), plus close-button and real route navigation. Comparison/inbox browser regressions, production build and 8,060 contrast samples also pass. **Remaining:** nested Marvin/evidence interactions, other required routes and whole-journey scans, manual/native-language/physical-device/Firefox/Safari review. See [focus contract](docs/NAVIGATION_ACCESSIBILITY.md). HL-097 remains in progress.
...
**8 September 2026 — document summary metadata:** Current-version metadata now uses the existing scoped scalar reader; text, passage arrays and artifact storage keys are no longer hydrated for document summaries. Scan/latest comparison selection reads only ID/mode/counts, with exact-law and organization checks and deterministic ties. Full diff loads only when an accessible saved AI attempt needs its existing freshness check; no-report summaries load neither Version nor Comparison bodies. See [contract and limits](docs/DOCUMENT_HISTORY.md#document-summary-metadata--hl-099-8-september-2026) and verification. **Still open:** selected diff/result bytes, SQL JSON-statistics and ranking cost, action-decision history, compatibility law-list fan-out and target-host capacity. HL-099 remains **IN PROGRESS**.

**8 September 2026 — matrix saved-history batches:** The impact matrix now selects latest comparison IDs and preferred saved-report metadata in SQL, processing at most 50 laws per history batch. Only each latest comparison and selected successful result load; archived attempts, plans and comparison bodies stay out of the reader. Current/stale/failed/unknown behavior, complete history eligibility, latest failed status and same-comparison evidence links remain intact. Explicit profile/watch/law/comparison/analysis scope also applies in privileged sessions. See [contract and limits](docs/IMPACT_MATRIX.md#saved-history-reads--hl-099-8-september-2026) and verification. **Still open:** complete matrix pagination/response bounds, selected diff/result bytes, SQL ranking/index cost and intended-host capacity. HL-099 remains **IN PROGRESS**.

**8 September 2026 — complete current-report selection:** Document/comparison reads now search the whole saved impact-attempt history for the newest successful current-cache report, then the newest previous success, then the latest attempt. The former last-50 window could hide a current report after many failed retries. Selection reads metadata/IDs and materializes one chosen report, preserving the separately displayed latest failure and all archived records. Organization, comparison and law visibility remain explicit, including privileged sessions; newer attempts wait for the next read. See [selection contract](docs/CURRENT_REPORT_SELECTION.md) and the verification record. **Still open:** SQL planner/index cost, current-version/comparison bodies, action-decision history bounds, other read surfaces and intended-host capacity. All six isolated PostgreSQL 16.14 selection/fallback/visibility scenarios passed after Docker became available; no application data was used. HL-099 remains **IN PROGRESS**.

**8 September 2026 — complete regulatory-timeline pages:** Both document detail and the standalone timeline now return 20-record pages for saved events/versions/comparisons, identifiers, expressions, relations and source observations, with complete cursor continuation and accessible totals. The five-language document UI provides separate list selectors, previous/next/retry/latest controls, evidence/source links and retained reading state during page errors. Scope is rechecked on every request; late event admission is separate from its detection timestamp. See [timeline paging contract](docs/REGISTRY.md#complete-regulatory-timeline-pages--8-september-2026) and the verification record. HappySnowman's source-pack recovery and dynamic fixture date are preserved during integration. **Still open:** individual metadata byte bounds, SQL count/ranking cost, selected summary/analysis reads, other compatibility full-list consumers and intended-host capacity. HL-099 remains **IN PROGRESS**; saved chronology does not establish live source coverage or a law's effective date.

**6 September 2026 — complete document-history pages:** The actual document page now opts into 20-record metadata pages for all saved versions, comparisons and fetch/import observations, with full keyset continuation, previous/retry/latest controls and accurate accessible-record counts. Older selected comparison/baseline versions and the current live version stay in the selectors across page navigation; viewers can browse without mutations. Current scope is checked on every page, including after membership/watch/ownership changes. Added supporting indexes without rewriting evidence and fixed invalid membership cookies causing an unhandled error instead of sign-in recovery. See [document-history contract](docs/DOCUMENT_HISTORY.md) and `docs/VERIFICATION.md`. **Still open:** regulatory-timeline paging, complete metadata byte bounds, count/query-planner cost, summary/analysis reads, compatibility full-list consumers and intended-host capacity. HL-099 remains **IN PROGRESS**; saved history does not prove live source coverage.

**6 September 2026 — law-detail history metadata:** The actual document detail now uses scalar queries for its version/comparison/observation lists. SQL preserves character/passage/page counts and extracts only comparison counts, avoiding historical text, passage-array and diff transfer. Existing metadata/evidence links and preview sizes remain compatible, with stable tie ordering and explicit watch/law/history scope. The populated HTTP regression checks 152 versions and 50/100 previews; only the current version and selected summary comparison still load as bodies. See [contract](docs/REGISTRY.md#law-detail-history-metadata--6-september-2026) and verification record. **Still open:** full history paging/preview disclosure, metadata-size and SQL aggregation cost, selected summary/analysis reads and intended-host capacity. HL-099 remains **IN PROGRESS**.

**6 September 2026 — scalar saved timelines:** Timeline reads now select displayed metadata only, count normalized versions in SQL, cap the existing observation preview at SQL selection and batch relationship/alias links in one query. Explicit organization/owner predicates cover law/watch/mapping/work/history and related timeline links, including privileged sessions; active/paused aliases and both relation directions remain supported. Large-history and 50-related-work fixtures retain nine SELECTs and zero ORM object loads; fallback uses four. See [timeline contract](docs/REGISTRY.md#scalar-saved-timelines--6-september-2026) and verification record. **Still open:** complete timeline/metadata paging and response bounds, the separate law-detail/version/comparison/summary readers, query-planner cost and target-host concurrency. HL-099 remains **IN PROGRESS**.

**6 September 2026 — shared visible-page registry details:** Discover's linked-law/expression/date expansion now uses three scalar queries for either one or 50 events. Direct/incoming/outgoing relation paths are deduplicated in SQL; visible law/mapping and organization-watch constraints remain explicit. Monitoring and Discover share typed date batching, preserving each event/version's provenance and excluding cross-entity ID collisions. The 50-work/151-path, duplicate-link, private-scope, shared-date and empty-page regressions pass with 43 affected tests and six PostgreSQL scenarios; see [registry details](docs/REGISTRY.md#shared-visible-page-details--6-september-2026). Per-row query multiplication is removed; complete single-work fan-out/response size, planner cost, timeline paging, live-cursor reconciliation and intended-host capacity remain open. HL-099 stays **IN PROGRESS**.

**6 September 2026 — monitored-document SQL pages:** Monitoring now selects latest activity per visible watched law in SQL, using the shared bounded candidate traversal with Discover. Visible-page comparison links, expression IDs and official dates are scalar/batched instead of per-watch full-object/history loads. A latest-work event index supports selection. Paused/unmapped watches, personal read state, private law/mapping/work boundaries, DST dates and comparison ties remain covered. See [monitoring read contract](docs/REGISTRY.md#monitored-document-sql-pages--6-september-2026) and `docs/VERIFICATION.md`. Derived-activity sorting cost, large metadata fan-out, full timelines, Discover link expansion and cross-request snapshot/reconciliation semantics remain open, as do intended-host capacity gates; HL-099 stays **IN PROGRESS**.

**6 September 2026 — registry event SQL pages:** Discover/events now uses scalar SQL filters and `(detected_at, id)` keysets, with at most 100 candidates per selection and only the visible page plus one matching lookahead retained. Unicode literal search continues through sparse batches; visible-row dates/links no longer require full event/work JSON bodies. Organization/user scope, related watches, Zurich date boundaries and a reversible composite-index migration pass six SQLite/PostgreSQL scenarios; 33 affected registry/evidence regressions pass. See [registry contract](docs/REGISTRY.md#event-list-sql-paging--6-september-2026) and `docs/VERIFICATION.md`. Monitored-document/timeline reads, visible-row relation/date fan-out, total sparse-search database cost, snapshot/late-admission policy and intended-host capacity remain open. The existing registry page-size contract stays 1–100; this does not claim the broader ≤50 inbox/feed acceptance gate. HL-099 remains **IN PROGRESS**.

**6 September 2026 — current-topic fan-out:** Today now shows five current topic matches with complete keyset continuation. Batched selection and bounded evidence-validation groups replace materializing all matches at once; scanning continues past stale/rejected candidates rather than hiding later valid topics. Detail pages preserve exact reasons/confidence/review links and repeat access checks. Topic and document lists share scope-aware retry/back navigation. The 201-topic traversal, 120-stale-prefix, PostgreSQL and five-language browser evidence is recorded in `docs/VERIFICATION.md` and `docs/INTEREST_FEED.md`. Total stale-candidate validation/ranking cost, large per-match evidence fields, related-law fan-out and intended-host capacity remain open. HL-099/076 remain **IN PROGRESS**.
...
**Implemented 6 September 2026 (administrator reprocessing interface):** Platform admin now links to a dedicated five-locale page for previewing retained-candidate rule changes, inspecting counts and labelled examples, explicitly applying a fresh recheck, and cancelling/resuming saved batches. The latest 20 maintenance runs are filtered before the list limit, carry precise Zurich timestamps/mode/rule metadata, and have reloadable result links. Completed current-rule previews with changes can open the apply confirmation; unchanged, obsolete, superseded or malformed results cannot. A browser-session request identity preserves the original rule/mode across a lost response and reload. The API recovers an already accepted old-rule job without creating a new one, or rejects an unaccepted obsolete intent before any mutation. **Still open:** new-pair discovery/backfill, historical AI reassessment, digest catch-up, independent semantic/domain and native-language review, target-host evidence. This closes the dedicated-UI follow-up in the entry below, not HL-100 as a whole. See [administrator workflow](docs/RELATION_IMPACT_ANALYSIS.md#administrator-reprocessing-page) and `docs/VERIFICATION.md`.

**Implemented 6 September 2026 (controlled retained-candidate reprocessing):** Platform administrators can preview or explicitly apply the current deterministic retrieval rules to retained candidate pairs through an admin API. A durable maintenance job processes 25 candidates per committed batch, resumes from its saved cutoff/cursor after cancellation or failure, reuses repeated request IDs and stops if the deployed rule changes. Rule-obsolete and rejected candidates cannot supply current AI conclusions; refreshed inputs invalidate prior reports without rewriting history, human reviews, official relations or deliveries. Unsupported leads are labelled as unsupported retrieval, not a legal no-impact judgment. Processing uses metadata and version IDs, not archived document bodies, and makes no AI or notification calls. **Still open:** dedicated administration UI, discovery/backfill of new candidate pairs, explicit historical AI reassessment, digest catch-up, independently reviewed semantic/domain acceptance and target-host evidence. HL-100 remains **IN PROGRESS**. See [operation and limits](docs/RELATION_IMPACT_ANALYSIS.md#controlled-retained-candidate-reprocessing--6-september-2026) and the verification record. Earlier dated remaining-work notes below describe their original milestone, not additional completion claims.

**Implemented 6 September 2026 (observed local runtime freshness):** Relation cache/queued execution now bind the model-manager gateway's actual runtime identity instead of inventory labels. Immutable identity survives a restart; changed weights, tokenizer/template, runtime, hardware/context or token-count protocol invalidate current history/inbox/digest selection. Partially known identity is valid only for the same deployment. Changed queued bindings fail before inference, unbound requests retain local warm-up, and repeated pending offline requests coalesce without reusing a completed unknown-model result. Recipient-scoped observations are checked against current public configuration; digest selection stores the binding, probes again before delivery, restarts on changes and leaves the period unconsumed when the local runtime is unavailable. Reads use bounded metadata probes, never generation; original reports/citations remain history. **Still open:** candidate refresh/backfill, digest catch-up, independent semantic/domain acceptance and target-hardware measurements. HL-100 remains **IN PROGRESS**. See [runtime contract and limits](docs/RELATION_IMPACT_ANALYSIS.md#observed-local-runtime-freshness--6-september-2026) and the verification record.

**Implemented 6 September 2026 (same-ID evidence corrections):** Database evidence revisions now bind candidate/event/work/version/legacy/official-relation inputs independently of unchanged IDs or content hashes. SQLite/PostgreSQL triggers cover stored-content corrections, including direct SQL, without treating operational timestamps as new evidence. Planner v4, request identity, current history/inbox selection and final digest reads share the compact binding; old reports/citations remain immutable history. Preparation races stop before inference; corrections during generation produce stale saved results. A migration epoch prevents counter reuse after downgrade/re-upgrade. Inbox checks remain scalar SQL rather than loading archived passages. **Follow-up:** runtime/artifact freshness on reads is covered by the entry above; candidate refresh/backfill, digest catch-up, independent semantic/domain acceptance and target-hardware measurements remain open. HL-100 remains **IN PROGRESS**. See [contract and migration boundary](docs/RELATION_IMPACT_ANALYSIS.md#same-id-evidence-corrections-hl-100-6-september-2026).

**Implemented 6 September 2026 (exact relation endpoints and replacement direction):** An official relation must connect this candidate's exact source/target work pair before it can supply authoritative AI evidence, inbox confirmation/urgency, replacement links or a successor-monitoring action. Unrelated, missing and self endpoints fail closed; incoming and outgoing relations remain valid, with the official `replaces` subject always the successor. Final digest selection drops urgency derived solely from an invalid pair. Relation-impact v4 makes older reports history-only without automatic inference or rewriting saved citations. Existing human reviews and explicit event importance remain independent. **95 API regressions, seven isolated PostgreSQL scenarios and the production build pass**; see the verification record. **Still open:** semantic entailment, same-version/evidence/runtime freshness, candidate refresh/backfill and independent quality gates. HL-100 remains **IN PROGRESS**.

**Implemented 6 September 2026 (official-relation corrections):** Planner v3 binds relation identity/state/type/authority/provenance/evidence fingerprint and subject/object/source-version IDs in both request identity and saved plans. History, inbox and final digest reads reject missing or changed bindings without inference or rewriting saved reports/citations. Corrected inputs produce a distinct explicit request; failed retries cannot revive the old report. Older reports lacking this provenance become history-only. Scalar SQL checks preserve bounded history selection; see `docs/RELATION_IMPACT_ANALYSIS.md` and `docs/VERIFICATION.md`. **Still open:** full same-version/evidence/runtime freshness, historical reprocessing, digest catch-up and independent quality gates. HL-100 remains **IN PROGRESS**.
...
**Implemented 5 September 2026 (profile freshness on reads):** Relation history, legacy/paged inbox and their shared digest selectors now exclude successful reports whose saved organization-profile revision no longer matches. Missing/malformed provenance fails closed; old text/evidence/history remains inspectable, and failed reanalysis cannot revive obsolete applicability. Official urgency/relations remain visible independently. The profile form invalidates relation-history and digest caches. **147 affected API regressions** and isolated PostgreSQL profile/history-batch checks pass; the 50-event/7,474-history-row scenario still uses four history selection/hydration queries and 111 loaded records. No read-time inference, jobs, notifications or historical rewrites. **Still open:** full evidence/prompt/settings/runtime freshness, historical candidate backfill and independent relevance/entailment gates. HL-100 stays **IN PROGRESS**.

**Implemented 5 September 2026 (generation request identity):** Relation-analysis cache/durable job identity now includes temperature, top-p, presence penalty, reasoning effort, JSON mode and product ID alongside existing model/context/runtime inputs. Saved plans retain the generation controls without credentials. Changing any generation control produces a distinct analysis; repeating it or rotating the API token reuses the completed request. Transport timeout/retries/concurrency intentionally do not invalidate successful answers. Historical reports stay immutable; no bulk inference is triggered. Read-time settings/runtime/evidence freshness remains open independently.

**Implemented 5 September 2026 (configuration freshness on reads):** Saved relation plans now carry a fingerprint of provider/product/endpoint/model/context/output and generation controls. History and both inbox readers require that fingerprint as well as profile/schema freshness; old conclusions, actions and AI severity stay history-only when settings differ. Credential rotation and transport knobs do not invalidate answers. Digest preview, preparation and final delivery use the recipient organization's resolved public configuration; changed settings restart bounded preparation and reject an obsolete completed selection before sending. Settings save/reset invalidates relation-history/digest client caches. No read-time inference or archived-payload rewrite. Reports predating this fingerprint conservatively become stale; a newly requested analysis uses a new cache identity. SQLite API, isolated PostgreSQL configuration/digest/history-batch checks and the production web build pass (see verification record). **Still open:** prompt/evidence/runtime-artifact freshness on reads, historical candidate backfill and independent relevance/entailment gates. HL-100 remains **IN PROGRESS**.


**Implemented 5 September 2026 (effective prompt freshness):** Relation request identity and saved plans fingerprint only the impact and repair instructions actually used by this analysis. History and SQL inbox selection reject missing/mismatched prompt provenance; changing chat/batch-synthesis/context-mode settings alone preserves a valid relation report and request reuse. Organization overrides take precedence over platform defaults by effective content, including reset/inheritance transitions. Digest preparation and final selection resolve the recipient's effective prompts, restart on a changed fingerprint and refuse stale completed selections before sending. Prompt save/reset invalidates relation/digest UI caches. Synthetic API regressions, PostgreSQL prompt/digest/history-batch checks and the web build pass; historical reports remain accessible, no automatic reanalysis is triggered. **Still open:** evidence/runtime-artifact freshness, historical candidate backfill and independent quality gates. HL-100 remains **IN PROGRESS**.


**Implemented 5 September 2026 (candidate document-version freshness):** Relation plans bind the candidate's source/target version IDs, including explicit known-absent IDs. History and correlated SQL inbox selection require these bindings to match the same organization candidate; changed or removed IDs make the old report history-only without erasing citations or spending inference tokens. Final digest reads drop obsolete AI-only severity even after an event was selected for delivery. Official urgency/confirmed facts remain independent. Planner v2 changes request identity for the next explicit/scheduled analysis; missing old bindings fail closed. API and isolated PostgreSQL version/digest/bounded-history regressions pass (see verification). **Still open:** changed evidence/metadata under an unchanged version ID, legacy fallback evidence while the candidate version ID remains absent, candidate refresh lag, official relation corrections, local runtime-artifact freshness and independent quality gates. This is not a claim of complete evidence freshness; HL-100 remains **IN PROGRESS**.
