...
**Observed 6 September 2026 during document-history QA:** the companion's entry effect still posts `/assistant/context` and initializes `/assistant/conversations` when its persisted `enabled` preference is false. These are existing metadata/conversation initialization requests, not question generation; history navigation itself adds no requests of this kind. **Open follow-up:** respect disabled/context-detached preferences before bootstrap, verify re-enabling and route/account changes, and avoid creating empty conversations for a disabled companion. Recorded separately from the history feature; no privacy or zero-write claim should ignore this initialization.
