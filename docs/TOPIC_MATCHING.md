...
Failed spooling transactions leave no partial jobs/outbox. Failed matching transactions leave the preceding checkpoint and matches intact. Cancellation/retry preserves progress, and successful batches reset the consecutive-failure budget. Source retrieval failure is not turned into a successful empty match result. Human-reviewed match validity when formerly matching evidence becomes a non-match and feed-wide coverage/freshness remain follow-up work; this slice does not certify source completeness or notification usefulness.
