...
- Published scope at the split: 9 active scenarios, 62 required tasks and 9 deferred tasks. Additional deployment work must be recorded in the active v2 backlog before implementation; the snapshot counts do not claim its completion.
...
**Current step:** record and complete the separate deployment task on the active v2 branch for HappyDucky02 and monitoring.helveticlens.ch, including proof of isolation and branch-triggered updates. Pollen Watch remains the first complete product scenario, followed through the stated delivery sequence.
