...
The initial court scope is the Swiss Federal Supreme Court, not every cantonal court. Its official site provides the leading-decisions database, a broad judgments database that is effectively complete from 2007, a “new decisions” date index, and RSS. No stable documented public JSON change API was identified, so the implementation uses a conservative source contract over the official latest/date index and decision HTML. Source review established that the sitemap declared by `robots.txt` contains ordinary website pages rather than decision records or yearly `lastmod` feeds. The published robots policy specifies a two-second crawl delay, which is a hard lower bound for fetch scheduling.
...
- [Swiss Federal Supreme Court robots policy](https://search.bger.ch/robots.txt) declares the website sitemap and the two-second crawl delay; source review found that the sitemap does not enumerate decisions.
