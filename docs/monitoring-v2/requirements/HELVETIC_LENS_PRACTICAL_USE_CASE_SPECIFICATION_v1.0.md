# HELVETIC LENS — PRACTICAL USE CASE SPECIFICATION
...
Цель этой спецификации — наложить на существующую функциональную архитектуру **Helvetic Lens** десять конкретных практических сценариев, которые можно реализовать без превращения продукта в десять отдельных приложений.

Helvetic Lens уже построен вокруг базовой цепочки:
...
После scan система может сохранять:
...
В существующей продуктовой структуре уже присутствуют:
...
Backend уже рассчитан на получение web/PDF/API-источников, нормализацию, version storage, diff и background processing.

Следовательно, задача не состоит в создании десяти независимых вертикалей.

Целевая модель:

> **10 Monitoring Templates поверх одного Change Intelligence Engine.**
...
Все десять use cases должны использовать один и тот же базовый functional pattern.
...
Helvetic Lens должен одинаково работать с тремя типами изменений.
...
- появился официальный warning;
- отменён поезд;
- закрыт участок дороги;
- опубликован tender;
- зарегистрирован trademark;
- появился новый auction.
...
Объект существовал раньше, но изменилось его состояние.
...
Уже известное событие было изменено.
...
Именно здесь существующие `version history + comparison + evidence` дают Helvetic Lens особенно сильную продуктовую ценность.

---

# 5. Common Monitoring Subject

Для любого кейса пользователь создаёт не технический `source`, а понятный ему **Monitoring Subject**.
...
Базовая универсальная структура:
...
Рекомендуемая универсальная lifecycle-модель:
...
Если источник изменяет уже просмотренное событие:
...
Каждый delivered event должен обязательно отвечать на вопрос:
...
`Relevance Reason` должен быть обязательным структурным полем события, а не дополнительным AI-текстом.
...
Дать человеку возможность сказать:

> **Tell me when an official warning affects where I live, work or regularly stay.**

Пользователь не должен постоянно проверять Alertswiss, MeteoSwiss или кантональные ресурсы.

Helvetic Lens должен сообщать только о тех изменениях, которые пересекаются с **конкретными локациями пользователя**.
...
- владелец второго жилья;
- семья;
- человек, регулярно работающий в другом городе;
- человек с детьми, посещающими школу в другой локации;
- владелец загородного дома;
- traveller внутри Швейцарии.
...
Пользователь может создать несколько локаций:
...
Source Packs могут расширяться без изменения use case.
...
Цель — не мониторить весь Swiss public transport, а отслеживать **конкретную поездку, линию, остановку или commute** пользователя.
...
Другие возможные Monitoring Subjects:
...
Alert создаётся при meaningful state transition, а не при каждом telemetry update.
...
Helvetic Lens должен обновить **тот же development**, а не создать новую независимую карточку.
...
Это главный результат use-case анализа.

Нет необходимости строить десять разных engines.

Существующему Helvetic Lens нужны четыре универсальных capabilities поверх уже существующего source/version/diff/review core.

---

## 19.1 CAP-01 — Monitoring Subject

Сегодня продукт главным образом мыслит через topics/documents/sources.

Нужно добавить универсальное:
...
Helvetic Lens должен уметь хранить не только:
