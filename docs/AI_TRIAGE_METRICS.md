...
This compatibility correction does not supply the independent labelled gold set or participant review required by the remaining HL-093 criteria. The separate [offline matching evaluator](SEMANTIC_MATCHING_EVALUATION.md) now reports hash-bound source/input predictions against independently supplied labels, with held-out splits, adjudication, missing-result coverage, denominators and language/source strata. Its matching scores cannot satisfy report entailment/action-usefulness or model-promotion gates. The checked-in walkthrough is explicitly synthetic and unreviewed.
